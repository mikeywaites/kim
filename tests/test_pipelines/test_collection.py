...
def test_marshal_nested_collection_allow_create():
    ...
    f = field.Collection(field.Nested("UserMapper", allow_create=True), name="users")
    output = {}
    mapper_session = get_mapper_session(data=data, output=output)
    f.marshal(mapper_session)
...
def test_marshal_nested_collection_default():
    ...
    data = {"id": 2, "name": "bob", "users": [{"id": "1", "name": "ignore this"}]}

    def getter(session):
        if session.data["id"] == "1":
            return user

    f = field.Collection(field.Nested("UserMapper", getter=getter), name="users")
    output = {}
    mapper_session = get_mapper_session(data=data, output=output)
    f.marshal(mapper_session)
...
def test_marshal_nested_collection_allow_updates():
    ...
    f = field.Collection(
        field.Nested("UserMapper", getter=getter, allow_updates=True), name="users"
    )
    output = {}
    mapper_session = get_mapper_session(data=data, output=output)
    f.marshal(mapper_session)
...
def test_marshal_nested_collection_allow_updates_in_place():
    ...
    f = field.Collection(
        field.Nested("UserMapper", allow_updates_in_place=True), name="users"
    )
    output = {"users": [user]}
    mapper_session = get_mapper_session(data=data, output=output)
    f.marshal(mapper_session)
...
def test_marshal_nested_collection_allow_updates_in_place_too_many():
    ...
    user = TestType(id="1", name="mike")
    data = {"id": 2, "name": "bob", "users": [{"name": "name1"}, {"name": "name2"}]}
...
def test_marshal_nested_collection_allow_updates_in_place_too_many_with_allow_create():
    ...
    f = field.Collection(
        field.Nested("UserMapper", allow_updates_in_place=True, allow_create=True),
        name="users",
    )
    output = {"users": [user]}
    mapper_session = get_mapper_session(data=data, output=output)
    ...
    assert output == {"users": [TestType(id="1", name="name1"), TestType(name="name2")]}
...
def test_serialize_nested_collection():
    ...
    assert output == {
        "users": [{"id": "1", "name": "mike"}, {"id": "2", "name": "jack"}]
    }
...
def test_marshal_collection_unique_on():
    ...
    class PostMapper(Mapper):
        ...
        readers = field.Collection(
            field.Nested(UserMapper, allow_create=True), unique_on="id"
        )

    data = {"readers": [{"id": "1", "name": "jack"}, {"id": "1", "name": "jack"}]}
    ...
    data = {"readers": [{"id": "1", "name": "jack"}, {"id": "2", "name": "mike"}]}
...
def test_marshal_collection_required_false():
    ...
    class PostMapper(Mapper):
        ...
        readers = field.Collection(
            field.Nested(UserMapper, allow_create=True), required=False
        )
