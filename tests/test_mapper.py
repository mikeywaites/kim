...
def test_new_mapper_sets_roles():
    ...
    class MyMapper(MapperBase):
        ...
        __roles__ = {"overview": ["email",]}

    class OtherMapper(MyMapper):

        __roles__ = {"private": ["id",]}
...
def test_mapper_serialize_raw():
    ...
    assert result == {"id": 2, "name": "bob", "user": {"id": "foo", "name": "bar"}}
...
def test_mapper_serialize_raw_nested_nested():
    ...
    assert result == {
        "id": 2,
        "name": "bob",
        "user": {"id": "foo", "name": "bar", "company": {"id": "bin", "name": "baz"}},
    }
...
def test_mapper_serialize_empty_nested_sets_null_default():
    ...
    assert result == {
        "id": "2",
        "name": "foo",
        "user": {"id": "3", "name": "foo", "company": None},
    }
...
def test_serialize_empty_nested_nested():
    ...
    mapper = MapperBase(obj1)
    result = mapper.serialize()
    assert result == {"id": "2", "name": "foo", "user": {}}
    ...
    assert result == {"id": 2, "name": "bob", "user": {}}
...
def test_polymorphic_on_with_string():
    ...
    mapper = MapperA(data={})
    assert isinstance(mapper._polymorphic_opts["polymorphic_on"], Field)
...
def test_mapper_with_invalid_collection_fields_sets_errors():
    ...
    data = {"id": 1, "name": "bob", "users": [{"name": 1, "id": "foo"}, {"name": 1}]}
